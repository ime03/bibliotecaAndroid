...
class DettagliPrestitoRivista : Fragment() {
    private var prestito: PrestitoRivista? = null
    ...
    override fun onCreateView(
        ...
        prestito = arguments?.getParcelable("prestitorivista") ?: PrestitoRivista()
        ...
        if(!prestito!!.dataRestituzione.equals(""))
        {
            ...
            btnRestituisci.setText("RIVISTA GIA' RESTITUITA")
        }

        val copertinaImageView = view.findViewById<ImageView>(R.id.copertinaPrestito)
        val rivistaId = prestito?.IdArticolo

        if (!rivistaId.isNullOrEmpty()) {
            val rivistaRef = FirebaseDatabase.getInstance().getReference("riviste").child(rivistaId)
            rivistaRef.addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    ...
                    if (!urlCopertina.isNullOrEmpty()) {
                        Glide.with(this@DettagliPrestitoRivista)
                            .load(urlCopertina)
    ...
    private fun impostaRestituzione(idPrestito: String) {
        ...
        val ref = FirebaseDatabase.getInstance().getReference("prestitiRiviste").child(idPrestito)
        ref.child("dataRestituzione").setValue(dataOdierna.toString())
    }

    private fun aggiornaDisponibilità(idRivista: String) {
